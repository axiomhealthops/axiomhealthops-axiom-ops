...
export default function IntakeDashboardPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [records, statusFilter, regionFilter, insuranceFilter, typeFilter, monthFilter, search]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', gap: 2, padding: '10px 20px 0', background: 'var(--bg)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          {TABS.map(tab => {
          ...
          {activeTab === 'patients' && (
            <div>
              <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                <input placeholder="Search patient, diagnosis, insurance…" value={search} onChange={e => setSearch(e.target.value)}
                  style={{ padding: '7px 12px', border: '1px solid var(--border)', borderRadius: 6, fontSize: 12, outline: 'none', background: 'var(--card-bg)', minWidth: 260 }} />
