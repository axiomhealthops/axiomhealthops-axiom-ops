...
export default function UserManagementPage() {
  ...
  async function loadData() {
    ...
    setUsers(u || []);
    setPages(p || []);
    // Build override map: { coordinator_id: { page_key: boolean } }
    const map = {};
    (o || []).forEach(ov => {
  ...
  async function addUser() {
    ...
    // Create auth user via Supabase admin (invites user)
    const { data: inviteData, error: inviteError } = await supabase.auth.admin?.inviteUserByEmail?.(newUser.email) || { data: null, error: { message: 'Admin API not available — add via Supabase dashboard' } };
    // Insert coordinator record (will link when user accepts invite)
    ...
    setMsg('User added. They will receive an email invite to set their password.');
  ...
  async function toggleOverride(coordinatorId, pageKey, currentVal) {
    ...
    if (existing !== undefined) {
      ...
      if (existing === currentVal) {
        ...
        await supabase.from('user_page_overrides').update({ granted: !existing }).eq('coordinator_id', coordinatorId).eq('page_key', pageKey);
      }
    } else {
      // Add new override
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      ...
      <div style={{ flex:1, overflow:'auto', padding:20, display:'flex', flexDirection:'column', gap:16 }}>
        ...
        {showAdd && (
          <div style={{ background:'var(--card-bg)', border:'1px solid var(--border)', borderRadius:12, padding:20 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 }}>
              ...
              <div>
                <div style={{ fontSize:11, fontWeight:600, color:'var(--gray)', marginBottom:4 }}>Regions (click to toggle)</div>
            ...
            <div style={{ marginTop:16, display:'flex', gap:8 }}>
              ...
              <div style={{ fontSize:11, color:'var(--gray)', alignSelf:'center' }}>User will receive an email to set their password</div>
            </div>
          </div>
        )}
        ...
        {filtered.map(user => {
          ...
          return (
            <div key={user.id} style={{ background:'var(--card-bg)', border:'1px solid var(--border)', borderRadius:12, overflow:'hidden' }}>
              {/* User header */}
              <div style={{ padding:'16px 20px', display:'flex', alignItems:'center', gap:16 }}>
                ...
                {isAdmin && user.id !== profile?.id && (
                  <div style={{ display:'flex', gap:8 }}>
                    ...
                    <button onClick={() => deactivateUser(user.id, user.is_active !== false)}
              ...
              {isEditing && (
                <div style={{ borderTop:'1px solid var(--border)', padding:20, background:'var(--bg)', display:'flex', flexDirection:'column', gap:16 }}>
                  ...
                  {/* Page overrides */}
                  <div>
                    <div style={{ fontSize:12, fontWeight:700, color:'var(--black)', marginBottom:8 }}>Page Access Overrides <span style={{ fontWeight:400, color:'var(--gray)' }}>— individual access beyond their role default</span></div>
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>
                      {pages.map(page => {
                        ...
                        return (
                          <button key={page.page_key}
                            onClick={() => toggleOverride(user.id, page.page_key, effective)}
                  ...
                  <div style={{ display:'flex', gap:8 }}>
                    <button onClick={() => updateUser(user.id, { role:editForm.role, regions:editForm.regions, team:editForm.team })}
                      disabled={saving}
