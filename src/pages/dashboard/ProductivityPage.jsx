...
// Build a lookup map: normalized pariox name -> DB clinician
function buildNameMap(clinicians) {
  var map = {};
  clinicians.forEach(function(c) {
    // Primary key: DB full_name (First Last) lowercased
    map[c.full_name.toLowerCase()] = c.id;
    // If pariox_name override exists, map that too
    if (c.pariox_name) {
      map[c.pariox_name.trim().toLowerCase()] = c.id;
      // Also map the normalized version of pariox_name
...
export default function ProductivityPage() {
  ...
  // Build name lookup map from clinicians
  var nameMap = useMemo(function() { return buildNameMap(clinicians); }, [clinicians]);
  var clinicianById = useMemo(function() {
    var m = {};
    clinicians.forEach(function(c) { m[c.id] = c; });
    return m;
  }, [clinicians]);
 
  // Aggregate visit stats per clinician ID
  var statsByClinicianId = useMemo(function() {
    var map = {};
    visits.forEach(function(v) {
      var rawName = (v.staff_name || '').trim();
      if (!rawName) return;
      // Try: raw pariox name, normalized first-last, lowercased
      ...
      if (s.includes('completed')) map[id].completed++;
      else if (s === 'missed (active)') map[id].missedActive++;
  ...
  var matchedCount = enriched.filter(function(c) { return c.done > 0 || c.stats.scheduled > 0; }).length;
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', background: 'var(--card-bg)', flexShrink: 0 }}>
          <div style={{ flex: 1, padding: '12px 20px', borderRight: '1px solid var(--border)', textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: 'var(--gray)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Full Time ({summary.ft.count}) \u2014 Target: 25/wk</div>
          ...
          <div style={{ flex: 1, padding: '12px 20px', borderRight: '1px solid var(--border)', textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: 'var(--gray)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Part Time ({summary.pt.count}) \u2014 Target: 15/wk</div>
          ...
          <div style={{ flex: 1, padding: '12px 20px', textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: 'var(--gray)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>PRN / 1099 ({summary.prn.count}) \u2014 Alert at: 10</div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '16px 20px' }}>
          ...
          <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '2.2fr 0.6fr 0.7fr 0.9fr 1.6fr 0.6fr 0.6fr 0.6fr 0.6fr 0.6fr 0.9fr', padding: '8px 20px', background: 'var(--bg)', borderBottom: '1px solid var(--border)', fontSize: 10, fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              <span>Clinician</span><span>Region</span><span>Disc.</span><span>Type</span>
              <span>Progress This Week</span><span>Done</span><span>Target</span>
              <span>Evals</span><span>Re-Assess</span><span>Note\u26A0</span><span>Status</span>
            </div>
            ...
            {filtered.map(function(c, i) {
              ...
              var hasData = c.stats.completed > 0 || c.stats.scheduled > 0 || c.stats.missed > 0;
              var hasMissedActive = c.stats.missedActive > 0;
              return (
                <div key={c.id} style={{ display: 'grid', gridTemplateColumns: '2.2fr 0.6fr 0.7fr 0.9fr 1.6fr 0.6fr 0.6fr 0.6fr 0.6fr 0.6fr 0.9fr', padding: '10px 20px', alignItems: 'center', borderBottom: '1px solid var(--border)', background: isAlert || hasMissedActive ? '#FFF5F5' : i % 2 === 0 ? 'var(--card-bg)' : 'var(--bg)' }}>
                  <div>
                    ...
                    {hasData ? (
                      <>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--gray)', marginBottom: 4 }}>
                          <span>{c.done} of {c.target} visits</span>
                        ...
                        <div style={{ height: 7, background: 'var(--border)', borderRadius: 999, overflow: 'hidden' }}>
                          <div style={{ height: '100%', width: Math.min(c.pct, 100) + '%', background: barColor, borderRadius: 999, transition: 'width 0.4s ease' }} />
                        </div>
                      </>
                    ) : (
                  ...
                  <span style={{ fontSize: 15, fontWeight: 700, fontFamily: 'DM Mono, monospace', color: hasData ? 'var(--black)' : 'var(--gray)' }}>{hasData ? c.done : '\u2014'}</span>
                  <span style={{ fontSize: 12, color: 'var(--gray)', fontFamily: 'DM Mono, monospace' }}>{c.target}</span>
                  <span style={{ fontSize: 13, fontWeight: 600, fontFamily: 'DM Mono, monospace', color: c.stats.evals > 0 ? '#1565C0' : 'var(--gray)' }}>{c.stats.evals > 0 ? c.stats.evals : '\u2014'}</span>
                  <span style={{ fontSize: 13, fontWeight: 600, fontFamily: 'DM Mono, monospace', color: c.stats.reassessments > 0 ? '#7C3AED' : 'var(--gray)' }}>{c.stats.reassessments > 0 ? c.stats.reassessments : '\u2014'}</span>
                  <span style={{ fontSize: 13, fontWeight: 700, fontFamily: 'DM Mono, monospace', color: hasMissedActive ? '#DC2626' : 'var(--gray)' }} title="Missed (Active) — note not submitted, cannot bill">
