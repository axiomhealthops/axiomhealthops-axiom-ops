...
export default function OnHoldRecoveryPage() {
  ...
  const [form, setForm] = useState({
    patient_name: '', region: '', hold_reason: '', hold_date: '', notes: ''
  });
  ...
  const rate = records.length > 0 ? Math.round((recovered.length / records.length) * 100) : 0;
      ...
      <div style={{ padding: '20px 28px', flex: 1, overflow: 'auto' }}>
        ...
        <div style={S.summaryRow}>
          <div style={S.summaryCard}>
            <div style={S.summaryLabel}>On Hold</div>
            <div style={{ ...S.summaryVal, color: 'var(--danger)' }}>{onHold.length}</div>
          </div>
          <div style={S.summaryCard}>
            <div style={S.summaryLabel}>Recovered</div>
            <div style={{ ...S.summaryVal, color: 'var(--green)' }}>{recovered.length}</div>
          </div>
          <div style={S.summaryCard}>
            <div style={S.summaryLabel}>Recovery Rate</div>
            <div style={{ ...S.summaryVal, color: 'var(--blue)' }}>{rate}%</div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div style={S.empty}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--black)' }}>No patients on hold</div>
                ...
                <div style={S.table}>
                  ...
                  {onHold.map(r => (
                    <div key={r.id} style={S.tableRow}>
                      <span style={S.cellName}>{r.patient_name}</span>
            ...
            {recovered.length > 0 && (
              <>
                ...
                <div style={S.table}>
                  ...
                  {recovered.map(r => (
                    <div key={r.id} style={{ ...S.tableRow, opacity: 0.65 }}>
                      <span style={S.cellName}>{r.patient_name}</span>
                      <span style={S.cell}>Region {r.region || '?'}</span>
                      <span style={S.cell}>{r.hold_reason || '—'}</span>
                      <span style={S.cell}>{r.hold_date || '—'}</span>
                      ...
                      <span style={S.cell}>
                        <span style={S.recoveredBadge}>Recovered</span>
                      </span>
                    </div>
                  ))}
                </div>
...
const S = {
  ...
  summaryLabel: { fontSize: 12, color: 'var(--gray)', fontWeight: 500, marginBottom: 4 },
  summaryVal: { fontSize: 28, fontWeight: 700, fontFamily: 'DM Mono, monospace' },
  formCard: {
    background: 'var(--card-bg)', border: '1px solid var(--border)',
    borderRadius: 12, padding: 20, marginBottom: 20,
  },
  formTitle: { fontSize: 14, fontWeight: 600, color: 'var(--black)', marginBottom: 14 },
  ...
  input: {
    ...
    fontSize: 13, background: 'var(--bg)', color: 'var(--black)', outline: 'none',
  },
  submitBtn: {
  ...
  cancelBtn: {
    padding: '9px 18px', background: 'none', color: 'var(--gray)',
  ...
  tableHeader: {
    display: 'grid',
    gridTemplateColumns: '2fr 0.8fr 1.2fr 1.5fr 1fr 1.2fr',
    padding: '10px 20px', fontSize: 11, fontWeight: 600, color: 'var(--gray)',
    textTransform: 'uppercase', letterSpacing: '0.05em',
    background: 'var(--bg)', borderBottom: '1px solid
